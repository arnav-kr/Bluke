...
- **P1 fixed, device validation required — custom key sounds:** Mechvibes V2 multi-file and V1 single-audio-sprite ZIP packs can be safely imported. V1 audio is decoded and split once with Android platform codecs, then preloaded into the existing low-latency `SoundPool`; no decoder dependency was added. Duplicate IDs are rejected without overwriting, imported packs can be deleted with confirmation, and deleting the active pack falls back to the saved built-in sound. Failed/stale custom selections also recover to built-in sounds.
...
- **P1 fixed, device validation required — input modes:** tap-drag uses one consistent 300 ms window and guaranteed button-up. The four modes are Keyboard, Touchpad, Gamepad, and Multimedia. The short-lived combined Keyboard + Touchpad and gyro Mouse modes were removed; stored selections migrate safely.
- **P1 fixed, host validation required — remote media UX:** Multimedia provides explicit Consumer Control and slideshow buttons in posture-aware layouts. Phone volume-key forwarding is opt-in, defaults off, and is active only while that mode is visible and the Activity is started. The Fn guide is now a compact, bottom-aligned horizontal strip that leaves the function row usable.
...
Post-refactor inventory additions/changed counts: `BlukeApplication.kt` 24 (process owner), `bluetooth/HidLifecycle.kt` 87 (state/retry/capability models), `bluetooth/HidRegistrationCoordinator.kt` 74 (facade-backed registration policy), `bluetooth/LatestRequestProcessor.kt` 26 (conflated cancellation policy), `bluetooth/GamepadReport.kt` 71 (pure HID gamepad packing and D-pad output policy), `ui/GamepadInput.kt` 39 (pure D-pad geometry), `data/LayoutRepository.kt` 65 (DataStore persistence/migration), `data/KeyboardThemeRepository.kt` 155 (custom-theme JSON persistence and upgrade migration), `ui/HomeViewModel.kt` 102 (immutable Bluetooth UI state), `ui/InputMode.kt` (four-mode registry and preference migration), `ui/MediaPresentationView.kt` and `ui/RemoteControlButtons.kt` (Multimedia controls and lifecycle-scoped hardware-volume option), `KeyboardThemesActivity.kt` 581 (theme library/live editor; **refactor candidate**), `ui/KeyboardThemes.kt` 77 (theme models/catalog), `sound/AudioSpriteConverter.kt` 215 (platform-codec sprite decode and WAV slicing), `sound/SoundPreferences.kt` 27 (sound-setting migration), `sound/CustomSoundPackRepository.kt` 320 (safe pack import/selection), `sound/KeyboardSoundSynthesizer.kt` 661 (preloaded audio-bank lifecycle/playback; **refactor candidate**), `ui/KeyboardCharacterLayouts.kt` 115 (logical output profiles), `ui/KeyboardLayouts.kt` 654 (keyboard geometry/key models; **refactor candidate**), `MainActivity.kt` 124, `BehaviorActivity.kt` 1,134 (**critical**), `BluetoothKeyboardManager.kt` 1,280 (**critical**), `GamepadView.kt` 2,868 (**critical**), `TouchpadView.kt` 1,050 (**critical**), and `HomeScreen.kt` 1,247 (**critical**). Test additions include input-mode migration, tap-drag policy, media-key routing, sound, HID, gamepad, and keyboard-theme coverage. The original `main` inventory above remains the audit baseline.
...
17. Separate the input cycle into Keyboard, Touchpad, Gamepad, and Multimedia — complete. Existing untouched three-mode defaults gain Multimedia; deliberate custom subsets remain deliberate. Removed `mouse` maps to Multimedia, while removed `keyboard_touchpad` selections are discarded with a Keyboard fallback when necessary.
18. Replace the gyro and combined modes with explicit Multimedia controls, and move the Fn guide away from the function row — complete; physical host and small-display validation remains.
...
| Runtime Native/Web switching. | A compact gamepad-toolbar control reads and writes the same persisted mode as Behavior settings. The manager's existing preference listener sends neutral reports around the policy swap. | The active mapping is visible where it is used and changes without disconnecting or re-pairing. | The compact landscape toolbar must be checked on the smallest supported display/font scale. |
| Unsupported OEM firmware message. | Exhausted synchronous `getProfileProxy()` or `registerApp()` command rejection is treated as likely incompatibility and opens an explicit screen with Retry. Binding/registration timeouts and host connection rejection do not. | Gives users of affected firmware a clear outcome while preserving the late-callback workaround. | Android exposes no HID Device support feature flag; this is a bounded-retry heuristic and must be validated on physical OEM builds. |

The gamepad decision is backed by the project's [issue #16](https://github.com/arnav-kr/Bluke/issues/16), the USB-IF [HID Usage Tables](https://www.usb.org/hid) (Hat Switch is Generic Desktop usage `0x39`), the [Linux gamepad specification](https://www.kernel.org/doc/html/latest/input/gamepad.html), Linux's current [`hid-input.c`](https://github.com/torvalds/linux/blob/master/drivers/hid/hid-input.c), and the W3C [Standard Gamepad layout](https://www.w3.org/TR/gamepad/#remapping). Compose changes follow the current Android guidance to defer state reads and use lambda modifiers; no new Compose dependency was introduced. Android documents `getProfileProxy()` only as `true` on success and `false` on error, and says `registerApp()` return value is not the registration state; the callback remains authoritative: [BluetoothAdapter.getProfileProxy](https://developer.android.com/reference/android/bluetooth/BluetoothAdapter#getProfileProxy(android.content.Context,%20android.bluetooth.BluetoothProfile.ServiceListener,%20int)), [BluetoothHidDevice.registerApp](https://developer.android.com/reference/android/bluetooth/BluetoothHidDevice#registerApp(android.bluetooth.BluetoothHidDeviceAppSdpSettings,%20android.bluetooth.BluetoothHidDeviceAppQosSettings,%20android.bluetooth.BluetoothHidDeviceAppQosSettings,%20java.util.concurrent.Executor,%20android.bluetooth.BluetoothHidDevice.Callback)).

**ASSUMPTION:** Repeated synchronous command rejection after the three-attempt ceiling is the best available app-level signal of incompatible firmware, not proof. The manager status therefore says the device “appears” incompatible; the screen explains the firmware uncertainty, retains Retry, and explicitly avoids brand/model claims.
...
### 7.5 Unsupported-state, keyboard-layout, and custom-sound follow-up (2026-09-24)

#### Unsupported HID state

The reported visual contradiction was deterministic in `HomeScreen`: `ProfileNotSupportedScreen` was selected in the content branch, but the sticky mode and Launch controls were rendered after that branch and therefore remained visible. A `LaunchedEffect` independently turned the same rejected-registration message into a toast. `BluetoothState.blocksInputLaunch()` is now the single policy for `BluetoothOff`, `Unsupported`, and `ProfileNotSupported`; the sticky controls are absent for all three. `ProfileNotSupported` also suppresses the duplicate toast because the full-screen state already contains the reason and Retry action. Pairing, binding, registration, and connection refusal remain non-blocking and do not get falsely classified as unsupported.
...
The registry contains US QWERTY, French AZERTY, German QWERTZ, Dvorak, Colemak, and Russian JCUKEN. This is an extensible baseline, not a claim to cover every international keyboard. The French profile intentionally omits accented/dead-key and AltGr legends that cannot truthfully be emitted as one unmodified HID usage to a US-QWERTY host. JCUKEN is different: Bluetooth HID cannot emit Cyrillic Unicode directly, so it displays the standard Russian legends while preserving physical HID usages; the host input source must also be Russian. Shift+Space cycles the registered typing layouts, consumes Space so it cannot reach the host, and shows a temporary switcher with the required host input source. The rendered layout is committed only after both chord keys are released so recomposition cannot interrupt an active multi-touch gesture. The JCUKEN reference used was the standard Microsoft layout documented on [Wikipedia](https://en.wikipedia.org/wiki/JCUKEN).

Regional variants differ in dead keys, modifier layers, ISO/JIS geometry, and input-method behavior; the current on-screen boards are ANSI-derived and do not contain the ISO extra key. The next safe expansion should add a geometry capability model (`ANSI`, `ISO`, `JIS`) and explicit modifier/output transforms instead of hardcoding more labels into KLE strings. Unicode LDML similarly separates physical key maps, modifier maps, and output transforms: [Unicode LDML Keyboard Layouts](https://www.unicode.org/reports/tr35/tr35-keyboards.html).
...
Bluke now follows that lifecycle using its existing low-latency `SoundPool`: import through Android's document picker, validate/extract in app-private storage, unload the previous bank, deduplicate audio files, wait for decoder completion, atomically publish only successfully decoded samples, and play by an explicit USB-HID-to-Mechvibes-standard-keycode map. ZIP import rejects absolute/traversal paths, limits entries to 512, individual files to 16 MiB, and total expanded data to 64 MiB. A pack whose normalized ID is already installed returns an explicit duplicate result and leaves both files and selection untouched. Imported rows expose a confirmed delete action; built-ins do not. Deleting the selected pack clears its custom selection so the saved built-in profile is restored on resume. The page also links directly to the Key sounds section of Quick-cycle choices. A missing pack or a pack with no decodable key-down sample clears the stale selection and rebuilds the built-in bank instead of leaving the keyboard silent. No storage permission and no new third-party dependency are required.
...
| Add six character profiles over existing ANSI boards. | Covers the requested major arrangements plus Russian JCUKEN with a testable registry. | Russian requires the host's Russian input source; ISO/JIS and full dead-key/AltGr layers remain future work. |
...
e: .../LookAndFeelActivity.kt:289:34 Syntax error: Unexpected tokens
...
### 7.7 Four-mode input separation and remote-control follow-up (updated 2026-09-26)

The input registry keeps four explicit IDs: Keyboard (`0`), Touchpad (`1`), Gamepad (`2`), and Multimedia (`4`). ID `3` remains intentionally unused after removing Keyboard + Touchpad. Any saved set exactly equal to the legacy three-mode set gains Multimedia; a deliberate custom subset is preserved. The removed `mouse` preference maps to Multimedia, while `keyboard_touchpad` is removed and falls back to Keyboard if it was the only enabled mode.
...
- Multimedia exposes explicit Mute, Volume Down/Up, Previous, Play/Pause, Next, Page Up/Down, Home/End, F5, Escape, and `B` controls. Media commands use the existing Consumer Control report and presentation commands use keyboard reports, so no descriptor revision or re-pair is introduced.
...
Layout, theme, and key-sound cycle membership no longer lives on a duplicate Quick-cycle screen. Each native list now has two coordinated controls: tapping a row selects the active item and includes it in the cycle, while its checkbox includes or excludes it from toolbar tapping. Excluding the active item immediately selects the first remaining enabled choice, and the final enabled item cannot be unchecked. Stored selections are reconciled when each list opens, so upgrades cannot leave a highlighted row outside its cycle. Long-pressing the corresponding keyboard toolbar control opens the matching native list. The remaining Quick-cycle destination configures input modes only.
